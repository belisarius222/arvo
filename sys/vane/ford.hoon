...
++  za                                                  ::  per event
  ...
  ++  deps-take                                         ::  take rev update
    ...
    ?~  hashes  this
    %_    $
        sup.bay  (~(del by sup.bay) i.hashes)
        hashes  t.hashes
        mow 
      %-  weld  :_  mow
      %+  turn  ~(tap in (~(get ju sup.bay) i.hashes))
  ...
  ++  zo
    ...
    ++  make                                            ::  reduce silk
      ...
      ::
        ...
        ::  XX is /? in use? If it is, we don't need the extra add:dep calls
        ::  ::  until /? is in use, any hoon may implicitly depend on arvo types
        ::  %+  add-dep:bo  [%beam [bek /sys/hoon] %x]
        ::  %+  add-dep:bo  [%beam [bek /sys/zuse] %z]
      ...
      ::
        ...
        =+  dap=(~(get by def.deh.bay) %beam n.p.kas %z)
        ?~  dap  ~&(flag-missed+n.p.kas p.q.rez)
        (~(uni in p.q.rez) u.dap)  ::  XX if this errors out, then try p.u.dap
